...
class TestSecurityMiddleware:
    ...
    def app_with_middleware(self, mock_auth_provider):
        ...
        from turboapi.security.dependencies import get_auth_provider
        ...
        @app.get("/protected")
        async def protected_endpoint(current_user: User = Depends(get_current_user)):  # noqa: B008
...
class TestSecurityIntegration:
    ...
    def complete_app(self):
        ...
        @app.get("/admin/users")
        async def list_users(admin_user: User = Depends(require_role("admin"))):  # noqa: B008
            return {"message": "User list", "admin": admin_user.username}

        @app.delete("/admin/users/{user_id}")
        async def delete_user(
            user_id: str,
            admin_user: User = Depends(require_permission("delete")),  # noqa: B008
        ):
